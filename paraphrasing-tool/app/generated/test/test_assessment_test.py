# coding: utf-8

"""
    Paraphrasing Tool API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.assessment_test import AssessmentTest


class TestAssessmentTest(unittest.TestCase):
    """AssessmentTest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> AssessmentTest:
        """Test AssessmentTest
        include_optional is a boolean, when False only required
        params are included, when True both required and
        optional params are included"""
        # uncomment below to create an instance of `AssessmentTest`
        """
        model = AssessmentTest()
        if include_optional:
            return AssessmentTest(
                identifier = '',
                title = '',
                tool_name = '',
                tool_version = '',
                sections = [
                    openapi_client.models.assessment_section.AssessmentSection(
                        identifier = '', 
                        title = '', 
                        items = [
                            openapi_client.models.item.Item(
                                identifier = '', 
                                title = '', 
                                response_declaration = openapi_client.models.response_declaration.ResponseDeclaration(
                                    identifier = '', 
                                    cardinality = '', 
                                    base_type = '', 
                                    correct_response = [
                                        ''
                                        ], ), 
                                item_body = openapi_client.models.choice_interaction.ChoiceInteraction(
                                    response_identifier = '', 
                                    shuffle = True, 
                                    max_choices = 56, 
                                    prompt = '', 
                                    choices = [
                                        openapi_client.models.simple_choice.SimpleChoice(
                                            identifier = '', 
                                            text = '', )
                                        ], ), )
                            ], )
                    ]
            )
        else:
            return AssessmentTest(
                identifier = '',
                title = '',
                tool_name = '',
                tool_version = '',
                sections = [
                    openapi_client.models.assessment_section.AssessmentSection(
                        identifier = '', 
                        title = '', 
                        items = [
                            openapi_client.models.item.Item(
                                identifier = '', 
                                title = '', 
                                response_declaration = openapi_client.models.response_declaration.ResponseDeclaration(
                                    identifier = '', 
                                    cardinality = '', 
                                    base_type = '', 
                                    correct_response = [
                                        ''
                                        ], ), 
                                item_body = openapi_client.models.choice_interaction.ChoiceInteraction(
                                    response_identifier = '', 
                                    shuffle = True, 
                                    max_choices = 56, 
                                    prompt = '', 
                                    choices = [
                                        openapi_client.models.simple_choice.SimpleChoice(
                                            identifier = '', 
                                            text = '', )
                                        ], ), )
                            ], )
                    ],
        )
        """

    def testAssessmentTest(self):
        """Test AssessmentTest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)


if __name__ == "__main__":
    unittest.main()
